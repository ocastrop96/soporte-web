...
--
-- Servidor: localhost
-- Tiempo de generación: 27-02-2022 a las 04:18:16
...
-- Procedimientos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `ACTUALIZAR_ACTIVIDAD`(IN `_fechaAct` DATE, IN `_horaAct` TIME, IN `_descAct` VARCHAR(255), IN `_lugarAct` INT(11), IN `_empAct` INT(11), IN `_idActividad` INT(11))
UPDATE rv_actividades SET fechaAct = _fechaAct,horaAct = _horaAct,descAct = _descAct,lugarAct = _lugarAct,empAct = _empAct WHERE idActividad = _idActividad$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `ACTUALIZAR_USUARIO`(IN `_oficinaId` INT(11), IN `_rolId` INT(11), IN `_dni` TEXT, IN `_nombres` TEXT, IN `_aPaterno` TEXT, IN `_aMaterno` TEXT, IN `_cuenta` TEXT, IN `_clave` TEXT, IN `_correo` TEXT, IN `_id_usuario` INT(11))
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `CREAR_USUARIO`(IN `_oficinaId` INT(11), IN `_rolId` INT(11), IN `_dni` TEXT, IN `_nombres` TEXT, IN `_aPaterno` TEXT, IN `_aMaterno` TEXT, IN `_cuenta` TEXT, IN `_clave` TEXT, IN `_correo` TEXT)
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `ELIMINAR_ACTIVIDAD`(IN `_idActividad` INT(11))
DELETE FROM rv_actividades WHERE idActividad = _idActividad$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GRAFICOS`()
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `LISTAR_ACTIVIDADES_2`(IN `_fechaFinalAct` DATE)
SELECT
	rv_actividades.idActividad, 
	date_format(rv_actividades.fechaAct,'%d/%m/%Y') as fechaAct,
	time_format(rv_actividades.horaAct,'%H:%i %p') as horaAct,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `LISTAR_ACTIVIDADES_3`(IN `_inicio` DATE, IN `_fin` DATE)
SELECT
rv_actividades.idActividad,
date_format( rv_actividades.fechaAct, '%d/%m/%Y' ) AS fechaAct,
time_format( rv_actividades.horaAct, '%H:%i %p' ) AS horaAct,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `LISTAR_ACTIVIDADES_EDIT`()
SELECT
	rv_actividades.idActividad, 
	date_format(rv_actividades.fechaAct,'%d/%m/%Y') as fechaAct,
	time_format(rv_actividades.horaAct,'%h:%i %p') as horaAct,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `LISTAR_ACTIVIDADES_WEB`()
SELECT
	rv_actividades.idActividad, 
	date_format(rv_actividades.fechaAct,'%d/%m/%Y') as fechaAct,
	time_format(rv_actividades.horaAct,'%H:%i %p') as horaAct,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `LISTAR_ACTIVIDADES_WEB_2`(IN `_fechaFinalAct` DATE)
SELECT
	rv_actividades.idActividad, 
	date_format(rv_actividades.fechaAct,'%d/%m/%Y') as fechaAct,
	time_format(rv_actividades.horaAct,'%H:%i %p') as horaAct,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `LISTAR_ACTIVIDADES_WEB_3`(IN `_inicio` DATE, IN `_fin` DATE)
SELECT
rv_actividades.idActividad,
date_format( rv_actividades.fechaAct, '%d/%m/%Y' ) AS fechaAct,
time_format( rv_actividades.horaAct, '%H:%i %p' ) AS horaAct,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `LISTAR_EMPLEADO_DATOS`(IN `_idEmpleado` INT(11))
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `LISTAR_LUGARES_ACT`()
SELECT idLugar,descLugar FROM rv_lugaresact ORDER BY descLugar ASC$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `LISTAR_USUARIOS_OFICINA`(IN `_ofiEmp` INT(11))
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `LISTAR_VISITAS_1`(IN `_ofidepVisitado` INT(11))
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `LISTAR_VISITAS_2`(IN `_ofidepVisitado` INT(11), IN `_fechaFinal` DATE)
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `LISTAR_VISITAS_3`(IN `_ofidepVisitado` INT(11), IN `_fechaInicial` DATE, IN `_fechaFinal` DATE)
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `LISTA_LUGARACTI_FORM`()
SELECT idLugar,descLugar FROM `rv_lugaresact` ORDER BY descLugar ASC$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `LISTA_LUGARACTI_FORM_DESC`(IN `_texto` TEXT)
SELECT idLugar,descLugar FROM `rv_lugaresact` WHERE descLugar LIKE CONCAT('%',_texto,'%') ORDER BY descLugar ASC$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `LIST_REGISTRADOR_VISITA`()
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `REGISTAR_VISITA_REG`(IN `_fechaVisita` DATE, IN `_vstNombre` TEXT, IN `_vstDoc` INT(11), IN `_vstNdoc` TEXT, IN `_vstEntidad` INT(11), IN `_vstMotivo` INT(11), IN `_empVisitado` INT(11), IN `_ofidepVisitado` INT(11), IN `_cargoVisitado` TEXT, IN `_lugarVst` INT(11), IN `_horaIngreso` TIME, IN `_registraIngreso` INT(11))
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `REGISTRAR_ACTIVIDAD`(IN `_fechaAct` DATE, IN `_horaAct` TIME, IN `_descAct` VARCHAR(255), IN `_lugarAct` INT(11), IN `_empAct` INT(11), IN `_usuregAct` INT(11))
INSERT INTO rv_actividades(fechaAct,horaAct,descAct,lugarAct,empAct,usuregAct) VALUES(_fechaAct,_horaAct,_descAct,_lugarAct,_empAct,_usuregAct)$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `REGISTRAR_LUGAR_ACT`(IN `_descLugar` TEXT)
INSERT INTO rv_lugaresact(descLugar) VALUES(_descLugar)$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `REGISTRAR_SALIDA_REG`(IN `_horaSalida` TIME, IN `_fechaHRegSalida` DATE, IN `_registraSalida` INT(11), IN `_estadoV` INT(11), IN `_idVisita` INT(11))
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `REGISTRO_NUEVO_LUGAR_ACT`(IN `_desclugar` TEXT)
INSERT INTO rv_lugaresact(descLugar) VALUES(_desclugar)$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `REPORTE_GENERAL_1`(IN `_fechaFinal` DATE)
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `REPORTE_GENERAL_2`(IN `_fechaInicial` DATE, IN `_fechaFinal` DATE)
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `VISITAS_WEB`()
BEGIN
SELECT vis.idVisita,vis.fechaVisita,date_format(vis.fechaVisita,'%d/%m/%Y') as fVisita,vis.vstNombre,tdoc.descTDoc,vis.vstNdoc,ent.descEntidad,vis.vstMotivo,mot.descMotivo,vis.empVisitado,emp.nombresEmp,vis.ofidepVisitado,of.descOficina,vis.cargoVisitado,vis.lugarVst,lug.descLugar,time_format(vis.horaIngreso,'%r') as hEntrada,time_format(vis.horaSalida,'%r') as hSalida,vis.estadoV,rest.descEstado FROM rv_visitas as vis inner join rv_tdoc as tdoc on vis.vstDoc = tdoc.idTdoc inner join rv_estadovisita as rest on vis.estadoV = rest.idEstado inner join rv_entidades as ent on vis.vstEntidad = ent.idEntidad inner join rv_motivos as mot on vis.vstMotivo = mot.idMotivo inner join rv_empleados as emp on vis.empVisitado = emp.idEmpleado inner join rv_oficinas as of on vis.ofidepVisitado = of.idOficina inner join rv_lugares as lug on vis.lugarVst = lug.idLugar WHERE fechaVisita BETWEEN DATE_SUB(NOW(), INTERVAL 7 DAY) AND NOW() order by idVisita desc;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `VISITAS_WEB_1`(IN `_fechaFinal` DATE)
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `VISITAS_WEB_2`(IN `_fechaInicial` DATE, IN `_fechaFinal` DATE)
BEGIN
...
CREATE TABLE IF NOT EXISTS `rv_actividades` (
...
CREATE TABLE IF NOT EXISTS `rv_empleados` (
...
CREATE TABLE IF NOT EXISTS `rv_entidades` (
...
CREATE TABLE IF NOT EXISTS `rv_estadovisita` (
...
CREATE TABLE IF NOT EXISTS `rv_lugares` (
...
CREATE TABLE IF NOT EXISTS `rv_lugaresact` (
  `idLugar` int(11) NOT NULL,
  `descLugar` text COLLATE utf8_spanish_ci NOT NULL
...
CREATE TABLE IF NOT EXISTS `rv_motivos` (
...
CREATE TABLE IF NOT EXISTS `rv_oficinas` (
...
CREATE TABLE IF NOT EXISTS `rv_tdoc` (
...
CREATE TABLE IF NOT EXISTS `rv_usuarios` (
...
CREATE TABLE IF NOT EXISTS `rv_visitas` (
...
--
ALTER TABLE `rv_entidades`
  MODIFY `idEntidad` int(11) NOT NULL AUTO_INCREMENT,AUTO_INCREMENT=126;
--
...
--
ALTER TABLE `rv_visitas`
  MODIFY `idVisita` int(11) NOT NULL AUTO_INCREMENT,AUTO_INCREMENT=260;
